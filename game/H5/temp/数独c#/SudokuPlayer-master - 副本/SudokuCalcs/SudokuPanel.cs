...
namespace SudokuCalcs
{
    ...
    public partial class SudokuPanel : UserControl
    {
        ...
        /// <summary>
        /// 剔除重复性选择的栈
        /// 用来在有多个候选数是放除了选择的候选数的其他候选数  类型包含候选数，x，y，value
        /// 只进不出
        /// </summary>
        ...
        #region 删除一个Peers中的已经确定的数字  //计算器 删除候选数
        ...
        #region 寻找可用的数值 只有方法没有逻辑 计算器，取值
        /// <summary>
        /// 确定后更新数值  设置数值 唯一值时
        /// </summary>
        /// <param name="value"></param>
        ...
        private void SetCorrcertNumber(int value, int x, int y)
        {
            ...
            ShowSudokuDelectNumber(value, x, y);//  删除候选数
        ...
        private bool LogRecoder(List<RecoderSudoku> Recoder)
        {
            ...
            {//多个时把其他添加到数组后选择第一个
                for (int i = 1; i < Recoder.Count; i++)
                {
                    ...
                    SudokuRecoderStack.Push(Recoder[i]);//添加其他候选数，因为循环是从1开始的

                }
                SetMessage("SudokuRecoderStack.length:" + SudokuRecoderStack.ToArray().Length, SudokuMessageType.PopStack);
                SetCorrcertNumber(Recoder[0].Value, Recoder[0].X, Recoder[0].Y);
                return true;
            }
            return false;
            ...
            for (int i = 0; i < 9; i++)
            {
                for (int j = 0; j < 9; j++)
                {
                    ...
                    if (t.Numbers.Count < MinCount)
                    {
                        ...
                        tmp.X = i;
                        tmp.Y = j;
                        MinCount = t.Numbers.Count;//候选数字数量
                    }
                }
            }
            return tmp;
        ...
        /// <summary>
        /// 选择数字，错误处理：为空时取出猜的数的最后一个把其他放回去再递归，不为空时取第一个把其他放备用候选数
        /// </summary>
        private void FindItem()
        {
            ...
            {//候选数为空时
                ...
                //虽然看不懂，不过应该是取出数据
                //大概可能是取出第一个然后其他再放回去
                RecoderSudoku r = SudokuRecoderStack.Pop();//最后一个
                SudokuLiveNumbersArry = r.SudokuLiveNumbersArry;//对全局变量赋值,这个全局变量是之前记录的整个地图
/*打印提示*/    SetMessage("[" + NumberToAscii(r.X) + "," + (r.Y + 1).ToString() + "] 位置在栈中存在可选值，出栈：" + r.Value.ToString(), SudokuMessageType.PopStack);
                
               List <RecoderSudoku> tmpList = new List<RecoderSudoku>();//用来传参的list，其中放RecoderSudoku
        ...
        #region 验证选定值是否合适，相当于判断是否有错误，不值得参考  计算器，验证
